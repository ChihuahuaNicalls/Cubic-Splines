...
def getPolynomHermite(xp, fp, dp):
    n = len(xp)
    if not (len(fp) == n and len(dp) == n):
        raise ValueError("Todos los arreglos (xp, fp, dp) deben tener la misma cantidad de datos")
    ...
    coeffHermite = [Q[0, i] for i in range(2 * n)]
...
def printHermite(coeffHermite, z_nodes):
    ...
    poly_terms = []
    for i in range(n):
        term = f"{coeffHermite[i]:.7f}"
        if i > 0:
            for j in range(i):
                term += f"*(x - {z_nodes[j]:.7f})"
    ...
    for i in range(n):
        ...
        for j in range(i):
            ...
            current_term_poly = np.poly1d(
                np.polymul(current_term_poly.coeffs, factor.coeffs)
            )

        hermite_poly_np = np.poly1d(
            np.polyadd(hermite_poly_np.coeffs, current_term_poly.coeffs)
        )
...
def inputs():
    ...
    while numPuntos < 10:
        try:
            xInp = input(
                f"Ingresa el valor de x para el punto {numPuntos + 1} (o '.' para terminar): "
            )
            if xInp == ".":
                break
            xVal = float(xInp)
            ...
            dInp = input(
                f"Ingresa el valor de f'(x) (derivada) para el punto {numPuntos + 1}: "
            )
            dVal = float(dInp)
    ...
    for i in range(len(xp) - 1):
        ...
        yInterval = [
            evaluatePolynomsSplines(x, xIni, xEnd, pCoefficientsSplines[i])
            for x in xInterval
        ]
        ...
        axs[0].plot(
            xInterval,
            yInterval,
            label=f"Spline P{i}(x) en [{xIni:.2f}, {xEnd:.2f}]",
            linestyle="--",
            linewidth=2,
        )
    ...
    axs[1].plot(
        x_hermite, y_hermite, label="Polinomio de Hermite", color="blue", linewidth=2
    )
